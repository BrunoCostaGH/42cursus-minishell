...
/*   Updated: 2023/10/05 19:17:36 by bsilva-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_fd_out(t_data *data, int s_id)
{
	...
	while (data->argv.type[id] || data->argv.type[id + 1])
	{
		...
		{
			open_file(data, data->argv.args[id + 1][0], 0100 | 02 | \
			01000, &data->file_io[1]);
...
static void	get_fd_in_2(t_data *data, const int *id, int *status)
{
	...
	else if (data->argv.type[*id] == REDR_INPUT)
	{
		...
		data->file_io[0] = open(data->argv.args[*id + 1][0], 00);
		if (data->file_io[0] == -1)
		{
...
void	reset_io(t_data *data)
{
	...
	data->file_io[1] = 0;
	dup2(data->std_io[0], 0);
	dup2(data->std_io[1], 1);
}
